...
export default class ClientContactsRequestList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      current_page: 1,
  ...
  listApi = async (data) => {
    let res = await ApiCallClient("fetchclientcontactreqlistClient", data);
  ...
  declineRequest = () => {
    ...
    if (errorCount === 0) {
      let data = {
        status: "2",
        clientcontactid:
          this.state.listData[this.state.curIndex].clientcontactid,
  ...
  viewPage = (item) => {
    if(this.state.listData[this.state.curIndex].userTypeId == 6 || this.state.listData[this.state.curIndex].userTypeId == 7){
      this.props.history.push({
        pathname: "/clientViewContactRequester",
        state: this.state.listData[this.state.curIndex],
      });
    } else {
      this.props.history.push({
        pathname: "/clientViewContactRequestPage",
        state: this.state.listData[this.state.curIndex],
      });
    }
      ...
      <React.Fragment>
        ...
        <div className="component-wrapper">
          <div className="listing-component-app">
            ...
            >
              {" "}
              <Link to="/clientDashboard">Dashboard</Link> / Contacts Request
            </div>
                            ...
                            <div className="tble-row">
                              ...
                              >
                                <tr>
                                  ...
                                  <td style={{ width: "10%" }}>
                                    {data.mobile == null || data.mobile == undefined || data.mobile == "" ? "N/A" : data.mobile}
                                  </td>
                                  <td style={{ width: "10%" }}>
                                    {data.createdBy}
                                  </td>
                                  ...
                                  >
                                    {(data.department == null || data.department == undefined || data.department == "" ? "N/A" : textTruncate(data.department))}
                                  </td>
                                  <td style={{width:"10%"}}>
                                      {data.userTypeId == 6 ? "Requester" : data.userTypeId == 7 ? "Supervisor" : "LEI"}
                                  </td>
                                  <td style={{ width: "15%" }}>
                                    {SetDateFormat(data.createDate)} |{" "}
                                    {SetTimeFormat(data.createDate)}
                                  </td>
