...
import { MenuItem, modalClasses } from "@mui/material";
...
export default class ReceivableInterpretationPage extends React.Component {
  constructor(props) {
    super(props);
    var today = new Date();
    ...
    this.state = {
      ...
      preview_invoiceNote: "",
      preview_clientStatus: "",
      preview_purchaseOrder: "",
  ...
  getListDetails = async (data) => {
    ...
    let res = await ApiCall("fetchInvoiceAllJobs", data);
    if (
  ...
  getinvoiceDetails = async (data) => {
    ...
    let res = await ApiCall("fetchAllInvoiceReceivable", data);
    if (
      res.error === ErrorCode.ERROR.ERROR.WITHOUT_ERROR &&
      res.respondcode === ErrorCode.ERROR.ERROR_CODE.SUCCESS
    ) {
  ...
  openPrevInvoiceModal = async () => {
    ...
    if (brr.length > 1) {
      ...
      //   consoleLog("dataresss::preview payable data", decodeDataPayable);
      //   this.setState({
      //     preview_payableItems: decodeDataPayable.data.payableItems,
      //   });
      // }
      ...
      let resData = await ApiCall("fetchInvoiceByJobId", { jobId: jobID, invoiceId: this.state.mainInvoiceId })
      // consoleLog("preview data", resData);
      if (
        resData.error === ErrorCode.ERROR.ERROR.WITHOUT_ERROR &&
        resData.respondcode === ErrorCode.ERROR.ERROR_CODE.SUCCESS
        ...
        let previewData = decodeData.data.invoiceData[0];
        let payableData = decodeData.data.payableItems;
        consoleLog("preview data of invoice", decodeData);
        if (
          previewData != null ||
          previewData != undefined ||
          previewData != [] ||
  ...
  openCreateInvoiceModal = async () => {
    ...
    if (brr.length < 1) {
      ...
      if (
        ...
        // consoleLog("dataresss:payable items:", decodeDataPayable);
        this.setState({
      ...
      for (let x = 0; x < brr.length; x++) {
        for (let y = 0; y < brr.length; y++) {
          if (x != y) {
            if (brr[x].clientName == brr[y].clientName) {
              isDuplicate = true;
            } else {
              isDuplicate = false;
            }
      ...
      // consoleLog("brr:length:", brr.length);
      // consoleLog("isDuplicate::", isDuplicate)
        ...
        if (brr.length > 1) {
          if (isDuplicate === true) {
            ...
            if (
              ...
              // consoleLog("payable:fetch payable items:", decodeData)
              this.setState({
  ...
  onCreateTemplate = async () => {
    ...
    let validateTemplateName = inputEmptyValidate(this.state.templateName);

    if (validateTemplateName === false) {
  ...
  fetchTemplateApi = async () => {
    ...
    // consoleLog("id template:::", this.state.selectedTemplateData)
  ...
  onSelectTemplate = () => {
    // consoleLog("selected temp id",this.state.selectedTemplateData)
    this.setState({
  ...
  onDeleteParticulars_Edit = async (item, index) => {
    ...
    // if(item.payableReceivableId == null || item.payableReceivableId == undefined || item.payableReceivableId == ""){
    //   let obj ={"payableReceivableId":item.payableReceivableId}

    // let res = await ApiCall("removePayableItems",obj);
    // if (
    //   res.error === ErrorCode.ERROR.ERROR.WITHOUT_ERROR &&
    //   res.respondcode === ErrorCode.ERROR.ERROR_CODE.SUCCESS
    // ) {
  ...
  onCreateInvoice = async () => {
    ...
    if (errorCount === 0) {
      ...
      let data = {
        ...
        invoiceUserId: this.state.clientId,
