...
export default class ReceivableTranslationPage extends React.Component {
  constructor(props) {
    super(props);
    var today = new Date();
  ...
  getinvoiceDetails = async (data) => {
    ...
    if (
      ...
      consoleLog("Bill Payload: invoice", payload);
  ...
  onChangeLimit_invoice = async (dat) => {
    this.setState({
      limit_invoice: parseInt(dat.value),
  ...
  onResetFilter = () => {
    ...
    if(this.state.isVerifiedJobs){
      ...
      this.getListDetails(MainInvoiceData);
