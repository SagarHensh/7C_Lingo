...
export default class EditClientContact extends Component {
  ...
  load = async () => {
    ...
    if (
      ...
      console.log("dept1stdata::", deptMainData);
      // this.setState({
      //   deptArrMain: deptMainData,
      // });
    }
    ...
    if (
      ...
      for (let i = 0; i < deptInfo.length; i++) {
        ...
        // deptMainArr.push(deptMainData[i].value);
      ...
      for (let h = 0; h < deptMainData.length; h++) {
        for (let m = 0; m < deptInfo.length; m++) {
          if (deptMainData[h].value === deptInfo[m].deptid) {
            deptObjData.push({
              label: deptInfo[m].department,
              value: deptInfo[m].deptid,
            });
          }
        }
      }
    ...
    if (
      ...
      this.setState({
        imagePath: IMAGE_STORE_PATH + clientContactInfo.profilePic,
      });
    }
  };
