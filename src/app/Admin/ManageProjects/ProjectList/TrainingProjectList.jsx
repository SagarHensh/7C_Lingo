...
export default class TrainingProjectList extends React.Component {
  constructor(props) {
    super(props);
  ...
  listApi = async (data) => {
    ...
    const res = await ApiCall("getAllProjectsV2", data);
    if (
      res.error === ErrorCode.ERROR.ERROR.WITHOUT_ERROR &&
      res.respondcode === ErrorCode.ERROR.ERROR_CODE.SUCCESS
    ) {
      const decodeData = Decoder.decode(res.data.payload);
      // consoleLog("Training Project List", decodeData.data);
      let arr = [];
      if (decodeData.data.projectList.length > 0) {
        arr = decodeData.data.projectList;
      }
    ...
    //..............for history training listing...............
  };

  historyApi = async (data) => {
    // consoleLog("reqData:history:", data)
    const resHistory = await ApiCall("getAllProjectsHistoryV2", data);
    if (
      ...
      //   consoleLog("Payload history data>>>", decodeData1.data);
      let historyDetails = [];
      ...
      // consoleLog("Total history list Page>>>", decodeData1.data);
      this.setState({
        historyList: historyDetails,
        total_historyPage: totalPage1,
      });
    }
  };
  ...
  onChangeLimit = (dat) => {
    if (this.state.isAlljobs) {
      ...
      let data = {
        limit: dat.value,
        offset: JSON.stringify(
          (this.state.current_page_history - 1) * parseInt(dat.value)
        ),
  ...
  exRigth = () => {
    if (this.state.isAlljobs) {
      ...
      let totalPage = this.state.total_historyPage;
      this.setState({
    ...
    return (
      <React.Fragment>
        ...
        <div className="component-wrapper vewaljobs">
          ...
          >
            <Link to="/adminDashboard">Dashboard</Link> / Projects / Training
          </div>
                                    ...
                                    <tr>
                                      ...
                                      <td style={{ width: "10%" }}>
                                        ...
                                        <React.Fragment>
                                          ...
                                          >
                                            <MenuItem onClick={this.goEditHistory}>
                                              View Details
                                            </MenuItem>
                                            {/* <MenuItem
                                                onClick={this.declineModal}
