...
export default class EditPermission extends React.Component {
    ...
    load = async () => {
        ...
        let mres = await ApiCall("getModuleList");
        if (
            mres.error === ErrorCode.ERROR.ERROR.WITHOUT_ERROR &&
            mres.respondcode === ErrorCode.ERROR.ERROR_CODE.SUCCESS
        ) {
            let payload = await Decoder.decode(mres.data.payload);
        ...
        let res = await ApiCall("getRoleDetails", data);
        if (
            ...
            arr.map((arData) => {
                if (arData.value === respObject.roleTypeId) {
                    obj = {
                        label: arData.label,
                        value: arData.value
                    }
            ...
            moduleDataArr.map((data, i) => {
                if (data.id === respObject.modules[i].moduleId) {
                    brr.push({
                        moduleId: data.id,
                        moduleName: data.name,
                        permission: respObject.modules[i].permission
                    })
                }
            })

            this.setState({
                roleName: respObject.roleName,
                modulesPermissionData: brr,
                selectedUserTypes: obj,
                userType: respObject.roleTypeId
            });
        }
    ...
    onAdd = async () => {
        ...
        if (errorCount === 0) {
            ...
            let res = await ApiCall("updateRole", data);
            if (
                res.error === ErrorCode.ERROR.ERROR.WITHOUT_ERROR &&
                res.respondcode === ErrorCode.ERROR.ERROR_CODE.SUCCESS
            ) {
                toast.success(AlertMessage.MESSAGE.ROLE.ROLE_UPDATE);
                return history.push("/adminRoles");
            } else if (res.error === ErrorCode.ERROR.ERROR_CODE.DUPLICATE_ROLE_NAME) {
        ...
        return (
            <React.Fragment>
                ...
                <div className="component-wrapper">
                    ...
                    >
                        {" "}
                        <Link to="/adminDashboard">Dashboard</Link> / <Link to="/adminRoles">Roles & Permission</Link> / Edit
                    </div>
